...
def monitor_all(disk_path: Optional[str] = None, *, send_one_email: bool = True) -> str:
    ...
    v = get_disk_usage(disk_selected)
    if v is not None:
        lvl = check_limits(float(v), float(d["soft"]), float(d["hard"]), f"Disk Usage (%) on {disk_selected}",
                           trigger_email=not send_one_email)
        results.append({"metric": f"Disk ({disk_selected})", "level": lvl, "current": float(v),
                        "soft": float(d["soft"]), "hard": float(d["hard"])})

    # Memory
    m = DEFAULTS["memory_usage"]
    mv = get_memory_usage()
    lvl = check_limits(float(mv), float(m["soft"]), float(m["hard"]), "Memory Usage (%)",
                       trigger_email=not send_one_email)
    results.append({"metric": "Memory", "level": lvl, "current": float(mv),
                    "soft": float(m["soft"]), "hard": float(m["hard"])})

    # Processes
    p = DEFAULTS["process_count"]
    pv = float(get_process_count())
    lvl = check_limits(float(pv), float(p["soft"]), float(p["hard"]), "Running Process Count",
                       trigger_email=not send_one_email)
    results.append({"metric": "Processes", "level": lvl, "current": float(pv),
                    "soft": float(p["soft"]), "hard": float(p["hard"])})
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Beispiele:\n"
            "  python monitoring1.py all                     # alle Checks, eine Sammelmail bei HARD\n"
    ...
    parser.add_argument("--one-email", action="store_true",
                        help="Bei 'all': Suppress pro-Metrik-Emails und nur eine Sammelmail senden.")
    ...
    soft = 0.0 if soft is None else float(soft)
    hard = 0.0 if hard is None else float(hard)

    monitor_data(metric, soft, hard, eff_path)
